  ...
  tools: [
    ...
    },
    {
      ...
      description:
        ...
        "handed to a running named peer over the ordinary addressed send. Every " +
        "result names the worker; the bridge never merges its work, never widens " +
        "its permissions, and refuses a delegation from a session already at the " +
        "hop budget (ANTIPHON_HOP_BUDGET, default 1). Collect with " +
        "antiphon_task(id, \"result\", wait).",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        ...
        "test summary), or cancel. Read-only except cancel. A handed task has no " +
        "worker here: result and cancel are refused for it, and only the peer can " +
        "be told to stop.",
      inputSchema: {
        type: "object",
        properties: {
...
mcp.setRequestHandler(CallToolRequestSchema, async (request) => {
  if (request.params.name === "antiphon_delegate" || request.params.name === "antiphon_task") {
    ...
    if (isDelegate) {
      ...
      const text = typeof answer?.text === "string" && answer.text
        ? answer.text
        : "Delegated; antiphon task status <id> follows it.";
  ...
  const outcome = typeof answer?.text === "string" && answer.text
    ? answer.text
    : toClaude
      ? `Sent to the Claude Code peer '${to}' — the bridge said nothing more; run antiphon status to see whether it was received.`
      : (typeof to === "string"
