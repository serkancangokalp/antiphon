  ...
  tools: [
    ...
    },
    {
      ...
      description:
        ...
        "`claude -p` session in this project, read-only by default, in a git " +
        "worktree of its own for a write task — or, with `to`, the task handed " +
        "to a running named peer over the ordinary addressed send. Every result " +
        "names the worker; the bridge never merges its work, never widens its " +
        "permissions, and refuses a delegation from a session already at the hop " +
        "budget (ANTIPHON_HOP_BUDGET, default 1). Collect with " +
      ...
      inputSchema: {
        ...
        properties: {
          ...
          task: {
            ...
            description: "read (default): the worker runs read-only in the project. write: it edits in its own git worktree and returns a diff; nothing is merged.",
          },
          timeout: {
            type: "integer",
    ...
    },
    {
      ...
      description:
        ...
        "tests.txt), or cancel. Read-only except cancel.",
      inputSchema: {
        type: "object",
        properties: {
