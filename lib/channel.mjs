  ...
  if (request.params.name === "antiphon_delegate" || request.params.name === "antiphon_task") {
    ...
    try {
      const execution = execFileAsync("python3", argv, {
        cwd: projectDir,
        timeout: isDelegate ? 60_000 : 320_000,
      ...
      throw new Error(redactPrivate(
        String(error?.stderr || error?.message || error).trim().replace(/^task: /, ""), 500));
    }
    if (isDelegate) {
      let answer = null;
    ...
    return { content: [{ type: "text", text: String(stdout).trim() }] };
  }
