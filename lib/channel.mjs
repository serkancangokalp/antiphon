...
import { automaticProofVerdict as sharedProofVerdict, redactPrivate,
         automaticNameFromDigest, classifyEndpoint, fieldSpelledInteger,
         parseStrictJsonRecord } from "./identity.mjs";
...
const TASK_STATES = new Set([
  ...
  "blocked", "outcome_unknown", "handed", "handing", "tracking_incomplete",
  "delivery_refused",
]);
const TASK_KINDS = ["claude", "codex"];
const TASK_CLASSES = ["read", "write"];
const TASK_V2_STATES = new Set([
  "handing", "tracking_incomplete", "delivery_refused",
]);
const TASK_TERMINAL_STATES = new Set([
  "completed", "failed", "cancelled", "timed_out", "blocked", "outcome_unknown",
]);
const TASK_CANCEL_STATES = new Set([
  "completed", "failed", "cancelled", "timed_out", "blocked",
]);
const TASK_START_RECOVERY_STATES = new Set([
  "git_completion_receipt_missing", "unknown",
]);
const TASK_RECORD_KEYS = [
  ...
  "started_at", "finished_at", "collected_at",
];
const TASK_LIVENESS_KEYS = [...TASK_RECORD_KEYS, "worker_liveness", "liveness_detail"];
const TASK_RECOVERY_KEYS = [...TASK_RECORD_KEYS, "start_recovery", "recovery_detail"];
const TASK_RESULT_KEYS = [
  "id", "state", "exit_code", "log_path", "log_tail", "worker",
];
const TASK_RESULT_OPTIONAL_KEYS = new Set([
  "worker_liveness", "liveness_detail", "start_recovery", "recovery_detail",
  "diff", "diff_path", "diff_missing", "tests",
]);
const TASK_RESULT_STATES = new Set([
  "accepted", "running", ...TASK_TERMINAL_STATES,
]);
const TASK_HANDOFF_STATES = new Set([
  "handed", ...TASK_V2_STATES,
]);
const TASK_MAX_TIME = 2 ** 40;
const TASK_MAX_INTEGER = 2 ** 53 - 1;
const TASK_MAX_PID = 2 ** 31 - 1;
const TASK_ACTION_TIMEOUT_MS = 420 * 1_000;
const TASK_OUTPUT_CEILING = 4 * 1024 * 1024;
const TASK_DIFF_CEILING = 256 * 1024;
const TASK_TESTS_CEILING = 256 * 1024;
const TASK_LOG_CEILING = 4_096;
const TASK_DETAIL_CEILING = 4_096;
const TASK_PATH_CEILING = 16 * 1024;
const TASK_DIAGNOSTIC_CEILING = 16 * 1024 + 512;
const TASK_BIRTH_CEILING = 80;

function finiteTime(value) {
  return typeof value === "number" && Number.isFinite(value)
    && value >= 0 && value <= TASK_MAX_TIME;
}
...
function boundedString(value, ceiling, { nonempty = false } = {}) {
  return typeof value === "string"
    && (!nonempty || value.length > 0)
    && Array.from(value).length <= ceiling;
}

function subprocessErrorText(error) {
  ...
  const fallback = error === undefined || error === null ? "" : String(error);
  return stderr.trim() || message.trim() || fallback.trim()
    || "bridge subprocess failed without a diagnostic";
}

function taskActionOutcomeUnknown(error) {
  return error?.code === "ERR_CHILD_PROCESS_STDIO_MAXBUFFER"
    || error?.code === "ETIMEDOUT"
    || typeof error?.code === "number"
    || error?.killed === true
    || (typeof error?.signal === "string" && Boolean(error.signal));
}

function spelledSafeInteger(wrapper, key, value, minimum, maximum) {
  return Number.isSafeInteger(value) && value >= minimum && value <= maximum
    && fieldSpelledInteger(wrapper, key);
}

function nullableSpelledInteger(wrapper, key, value, minimum, maximum) {
  return value === null
    || spelledSafeInteger(wrapper, key, value, minimum, maximum);
}

function validTaskRecord(wrapper, expectedId, {
  allowLiveness = false, allowRecovery = false,
} = {}) {
  const answer = wrapper?.record;
  const liveness = Object.prototype.hasOwnProperty.call(answer ?? {}, "worker_liveness")
    || Object.prototype.hasOwnProperty.call(answer ?? {}, "liveness_detail");
  const recovery = Object.prototype.hasOwnProperty.call(answer ?? {}, "start_recovery")
    || Object.prototype.hasOwnProperty.call(answer ?? {}, "recovery_detail");
  const expectedKeys = liveness ? TASK_LIVENESS_KEYS
    : recovery ? TASK_RECOVERY_KEYS : TASK_RECORD_KEYS;
  if (!hasExactKeys(answer, expectedKeys)
      || (liveness && recovery)
      || (liveness && !allowLiveness)
      || (recovery && !allowRecovery)
      || answer.id !== expectedId
      || !DELIVERY_ID_PATTERN.test(answer.id)
      || !TASK_STATES.has(answer.state)
      || answer.version !== (TASK_V2_STATES.has(answer.state) ? 2 : 1)
      || !TASK_KINDS.includes(answer.kind)
      || !TASK_CLASSES.includes(answer.task_class)
      || typeof answer.sha256 !== "string" || !IDENTITY_DIGEST_PATTERN.test(answer.sha256)
      || !spelledSafeInteger(wrapper, "version", answer.version, 1, 2)
      || !spelledSafeInteger(wrapper, "size", answer.size, 0, TASK_MAX_INTEGER)
      || !nullable(answer.parent,
        (value) => typeof value === "string" && DELIVERY_ID_PATTERN.test(value))
      || !spelledSafeInteger(wrapper, "timeout", answer.timeout, 1, 3_600)
      || !spelledSafeInteger(wrapper, "hop", answer.hop, 0, TASK_MAX_INTEGER)
      || !finiteTime(answer.created_at)
      || !nullableSpelledInteger(
        wrapper, "pid", answer.pid, 1, TASK_MAX_PID)
      || !nullable(answer.birth,
        (value) => boundedString(value, TASK_BIRTH_CEILING, { nonempty: true }))
      || !nullable(answer.base,
        (value) => typeof value === "string" && /^[0-9a-f]{40}$/.test(value))
      || !nullableSpelledInteger(
        wrapper, "exit_code", answer.exit_code,
        -TASK_MAX_INTEGER, TASK_MAX_INTEGER)
      || (TASK_HANDOFF_STATES.has(answer.state)
        ? typeof answer.to !== "string" || !NAME_PATTERN.test(answer.to)
        : answer.to !== null)
      || !nullable(answer.started_at, finiteTime)
      || !nullable(answer.finished_at, finiteTime)
      || !nullable(answer.collected_at, finiteTime)) {
    return false;
  }
  if (liveness) {
    return answer.state === "running"
      && answer.worker_liveness === "unknown"
      && boundedString(answer.liveness_detail, TASK_DETAIL_CEILING,
        { nonempty: true })
      && Boolean(answer.liveness_detail.trim());
  }
  if (recovery) {
    return answer.state === "accepted"
      && TASK_START_RECOVERY_STATES.has(answer.start_recovery)
      && boundedString(answer.recovery_detail, TASK_DETAIL_CEILING,
        { nonempty: true })
      && Boolean(answer.recovery_detail.trim());
  }
  return true;
}

function validTaskResult(wrapper, expectedId) {
  const answer = wrapper?.record;
  if (!answer || typeof answer !== "object" || Array.isArray(answer)
      || !TASK_RESULT_KEYS.every((key) => Object.prototype.hasOwnProperty.call(answer, key))
      || Object.keys(answer).some((key) =>
        !TASK_RESULT_KEYS.includes(key) && !TASK_RESULT_OPTIONAL_KEYS.has(key))
      || answer.id !== expectedId || !DELIVERY_ID_PATTERN.test(answer.id)
      || !TASK_RESULT_STATES.has(answer.state)
      || !nullableSpelledInteger(
        wrapper, "exit_code", answer.exit_code,
        -TASK_MAX_INTEGER, TASK_MAX_INTEGER)
      || !boundedString(answer.log_path, TASK_PATH_CEILING, { nonempty: true })
      || !boundedString(answer.log_tail, TASK_LOG_CEILING)
      || !hasExactKeys(answer.worker,
        ["kind", "name", "directory", "work", "task_class"])
      || !TASK_KINDS.includes(answer.worker.kind)
      || answer.worker.name !== `worker-${expectedId.slice(0, 8)}`
      || !boundedString(
        answer.worker.directory, TASK_PATH_CEILING, { nonempty: true })
      || !TASK_CLASSES.includes(answer.worker.task_class)
      || (answer.state === "accepted"
        ? answer.worker.work !== null
        : answer.worker.task_class === "read"
          ? !nullable(answer.worker.work,
            (value) => boundedString(value, TASK_PATH_CEILING, { nonempty: true }))
          : !boundedString(answer.worker.work, TASK_PATH_CEILING, { nonempty: true }))) {
    return false;
  }
  if (["accepted", "running", "outcome_unknown"].includes(answer.state)
      && answer.exit_code !== null) return false;
  const livenessKeys = ["worker_liveness", "liveness_detail"]
    .filter((key) => Object.prototype.hasOwnProperty.call(answer, key));
  const recoveryKeys = ["start_recovery", "recovery_detail"]
    .filter((key) => Object.prototype.hasOwnProperty.call(answer, key));
  if (livenessKeys.length !== 0 && livenessKeys.length !== 2) return false;
  if (recoveryKeys.length !== 0 && recoveryKeys.length !== 2) return false;
  if (livenessKeys.length && recoveryKeys.length) return false;
  if (livenessKeys.length === 2
      && (answer.state !== "running" || answer.worker_liveness !== "unknown"
        || !boundedString(answer.liveness_detail, TASK_DETAIL_CEILING,
          { nonempty: true })
        || !answer.liveness_detail.trim())) {
    return false;
  }
  if (recoveryKeys.length === 2
      && (answer.state !== "accepted"
        ...
        || !boundedString(answer.recovery_detail, TASK_DETAIL_CEILING,
          { nonempty: true })
        || !answer.recovery_detail.trim())) {
    return false;
  }
  const evidenceKeys = ["diff", "diff_path", "diff_missing"]
    .filter((key) => Object.prototype.hasOwnProperty.call(answer, key));
  const completedWrite = answer.state === "completed"
    && answer.worker.task_class === "write";
  if (evidenceKeys.length !== (completedWrite ? 1 : 0)) return false;
  if (Object.prototype.hasOwnProperty.call(answer, "diff")
      && !boundedString(answer.diff, TASK_DIFF_CEILING)) return false;
  if (Object.prototype.hasOwnProperty.call(answer, "diff_path")
      && !boundedString(answer.diff_path, TASK_PATH_CEILING,
        { nonempty: true })) return false;
  if (Object.prototype.hasOwnProperty.call(answer, "diff_missing")
      && !boundedString(answer.diff_missing, TASK_DIAGNOSTIC_CEILING,
        { nonempty: true })) return false;
  if (Object.prototype.hasOwnProperty.call(answer, "tests")
      && (!TASK_TERMINAL_STATES.has(answer.state)
        || !boundedString(answer.tests, TASK_TESTS_CEILING))) return false;
  return true;
}
...
const mcp = new Server(
  { name: "antiphon", version: packageVersion() },
  {
    ...
    instructions:
      ...
      "status` shows the receipt. An event with sender=\"claude\" is another " +
      "Claude session's words; answer it with reply_to_claude(to=…) or " +
      "`@claude:name` — always named, not a lane of the passive page. " +
      "To hand one task to a fresh worker of either kind, call " +
      "antiphon_delegate and follow it with antiphon_task; the bridge never " +
      "merges its work. " +
      "`<unnamed>` means that peer " +
  ...
  tools: [
    {
      ...
      description:
        ...
        "a bare reply refused. The result says queued, never delivered. A lost " +
        "transport acknowledgement returns an unknown outcome and says not to retry, " +
        "unless the peer's transcript already proves receipt, in which case it returns received.",
      inputSchema: {
        type: "object",
        properties: {
          text: {
    ...
    },
    {
      ...
      description:
        ...
        "antiphon status shows when its transcript received it. A lost transport " +
        "acknowledgement returns an unknown outcome and says not to retry, unless " +
        "the peer's transcript already proves receipt, in which case it returns received.",
      inputSchema: {
        type: "object",
        properties: {
          text: {
    ...
    },
    {
      ...
      description:
        "Delegates one task to a worker of the other kind (Codex, or a kind you " +
        "name) and returns at once with a task id: a fresh `codex exec` / " +
        "`claude -p` session, read-only by default, working in a detached git " +
        "worktree of its own at HEAD whenever this project is a checkout with a " +
        "commit — for a read task as for a write task, so it sees committed work " +
        "and not your uncommitted changes; in a plain directory a read task runs " +
        "in the project and a write task is refused. Or, with `to`, a read task " +
        "handed to a running named peer over the ordinary addressed send. A hand-off " +
        "uses the same attempt id for task and delivery; if post-send tracking is " +
        "incomplete, its result says not to retry because the peer may already act. A " +
        "fresh-worker result names the worker and is collected with antiphon_task; a " +
        "hand-off result names the peer and is not collected by id — its answer arrives " +
        "in that peer's own words. The bridge never merges its work, never widens " +
        "its permissions, and refuses a delegation from a session already at the " +
        "hop budget (ANTIPHON_HOP_BUDGET, default 1).",
      inputSchema: {
        type: "object",
        properties: {
          ...
          kind: {
            ...
            description: "The worker's kind; codex by default from this side.",
          },
          to: {
            type: "string",
            description: "Hand the task to this running named peer of `kind` instead of starting a worker.",
          },
          task: {
            type: "string", enum: ["read", "write"],
            description: "read (default): under the host's read-only class, in the worker's own worktree at HEAD (in the project itself only when it is not a checkout). write: it edits in its own worktree at HEAD and returns a diff; nothing is merged, a commit is needed, and it cannot be handed with `to`.",
          },
          timeout: {
            type: "integer",
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          wait: { type: "integer", description: "For result: seconds to wait (0 to 300)." },
        },
        required: ["id", "action"],
      },
    },
...
mcp.setRequestHandler(CallToolRequestSchema, async (request) => {
  if (request.params.name === "antiphon_delegate" || request.params.name === "antiphon_task") {
    // Both are the Python bridge's `task` command; this server adds only its
    // own side and the alias it won, the trust `reply` extends already.
    const isDelegate = request.params.name === "antiphon_delegate";
    ...
    if (isDelegate) {
      if (typeof args.text !== "string" || !args.text.trim()) {
        throw new Error("Not delegated: the task text is empty");
      }
      expectedKind = Object.prototype.hasOwnProperty.call(args, "kind")
        ? args.kind : "codex";
      if (!TASK_KINDS.includes(expectedKind)) {
        throw new Error("Not delegated: name a kind (claude or codex)");
      }
      expectedClass = Object.prototype.hasOwnProperty.call(args, "task")
        ? args.task : "read";
      if (!TASK_CLASSES.includes(expectedClass)) {
        throw new Error("Not delegated: task must be read or write");
      }
      if (args.timeout !== undefined
          && (typeof args.timeout !== "number" || !Number.isFinite(args.timeout))) {
        throw new Error("Not delegated: timeout must be a finite number of seconds");
      }
      await identitySettled;
      argv = [bridgeScript, "task", "delegate"];
      input = JSON.stringify({
        text: args.text, kind: args.kind, to: args.to, task: args.task,
        timeout: args.timeout, side: "claude", sender_alias: senderAlias,
      });
    } else {
      const action = args.action;
      if (!["status", "result", "cancel"].includes(action)) {
        throw new Error("action must be status, result or cancel");
      }
      if (typeof args.id !== "string" || !args.id) {
        throw new Error("unknown task id");
      }
      argv = [bridgeScript, "task", action, args.id];
      if (action === "result") argv.push(String(Number.isFinite(args.wait) ? args.wait : 0));
    }
    let stdout = isDelegate ? "" : Buffer.alloc(0);
    try {
      const execution = execFileAsync("python3", argv, {
        cwd: projectDir,
        timeout: isDelegate ? 60_000 : TASK_ACTION_TIMEOUT_MS,
        maxBuffer: isDelegate ? 2 * 1024 * 1024 : TASK_OUTPUT_CEILING,
        encoding: isDelegate ? "utf8" : "buffer",
      });
      execution.child.stdin.on("error", () => {});
      execution.child.stdin.end(input ?? "");
      ({ stdout } = await execution);
    } catch (error) {
      const diagnostic = subprocessErrorText(error);
      const protocolRefusal = error?.code === 1
        && error?.killed !== true
        && !error?.signal
        && diagnostic.startsWith("task: ");
      let detail = diagnostic.replace(/^task: /, "");
      if (!isDelegate && !protocolRefusal && taskActionOutcomeUnknown(error)) {
        detail = `task action outcome is unknown; do not retry automatically: ${detail}`;
      }
      throw new Error(redactPrivate(detail, 500));
    }
    if (isDelegate) {
      let answer = null;
      try { answer = JSON.parse(String(stdout).trim().split("\n").pop()); } catch { answer = null; }
      const taskId = answer?.task_id;
      const canonicalId = typeof taskId === "string" &&
        DELIVERY_ID_PATTERN.test(taskId);
      const common = answer && typeof answer === "object" && !Array.isArray(answer) &&
        canonicalId && answer.kind === expectedKind &&
        answer.task_class === expectedClass && typeof answer.text === "string" &&
        Boolean(answer.text.trim());
      const incompleteHandoffStates = new Set(["handing", "tracking_incomplete", "missing"]);
      const handedKeys = ["task_id", "delivery_id", "state", "to", "kind",
        "task_class", "text"];
      const incompleteKeys = [...handedKeys, "tracking"];
      const incompleteUnknownKeys = [...incompleteKeys, "delivery_recorded"];
      const handoffState = answer?.state === "handed"
        ? hasExactKeys(answer, handedKeys)
        : incompleteHandoffStates.has(answer?.state)
          && answer.tracking === "incomplete"
          && (hasExactKeys(answer, incompleteKeys)
            || (hasExactKeys(answer, incompleteUnknownKeys)
              && typeof answer.delivery_recorded === "boolean"));
      const freshKeys = ["task_id", "state", "kind", "task_class", "timeout",
        "worker", "text"];
      const workerKeys = ["kind", "name", "directory"];
      const valid = args.to
        ? common && answer.to === args.to && answer.delivery_id === taskId &&
          handoffState
        : common && hasExactKeys(answer, freshKeys) && answer.state === "running" &&
          hasExactKeys(answer.worker, workerKeys) &&
          answer.worker.kind === expectedKind &&
          answer.worker.name === `worker-${taskId.slice(0, 8)}` &&
          typeof answer.worker.directory === "string" &&
          Boolean(answer.worker.directory.trim()) &&
          Number.isInteger(answer.timeout) && answer.timeout >= 1 && answer.timeout <= 3_600;
      if (!valid) {
        throw new Error("invalid delegation response from the bridge; no task was claimed");
      }
      return { content: [{ type: "text", text: JSON.stringify(answer, null, 1) }] };
    }
    const wrapper = parseStrictJsonRecord(stdout);
    const answer = wrapper.state === "valid" ? wrapper.record : null;
    const taskId = args.id;
    const valid = args.action === "status"
      ? validTaskRecord(wrapper, taskId, {
        allowLiveness: true, allowRecovery: true,
      })
      : args.action === "cancel"
        ? validTaskRecord(wrapper, taskId) && TASK_CANCEL_STATES.has(answer?.state)
        : validTaskResult(wrapper, taskId);
    if (!valid) {
      throw new Error(
        "invalid task response from the bridge; the action outcome is unknown; "
  ...
  let answer = null;
  try { answer = JSON.parse(String(stdout).trim().split("\n").pop()); } catch { answer = null; }
  const expectedTo = typeof to === "string" ? to : null;
  const common = answer && typeof answer === "object" && !Array.isArray(answer)
    && typeof answer.id === "string" && DELIVERY_ID_PATTERN.test(answer.id)
    && typeof answer.text === "string" && Boolean(answer.text.trim())
    && answer.to === expectedTo
    && typeof answer.recorded === "boolean"
    && typeof answer.dedupe_recorded === "boolean"
    && (answer.attachment === null
      || (typeof answer.attachment === "string"
        && ATTACHMENT_NAME_PATTERN.test(answer.attachment)));
  const uncertainKeys = ["outcome", "id", "to", "attachment", "recorded",
    "dedupe_recorded", "text"];
  const uncertain = common
    && hasExactKeys(answer, uncertainKeys)
    && (answer.outcome === "unknown"
      || (answer.outcome === "received" && answer.recorded === true));
  const successKeys = toClaude
    ? ["queued", "delivered", "id", "proof", "to", "attachment", "recorded",
      "dedupe_recorded", "text"]
    : ["queued", "id", "proof", "to", "attachment", "recorded",
      "dedupe_recorded", "text"];
  const success = common && answer.outcome === undefined && (toClaude
    ? hasExactKeys(answer, successKeys)
      && answer.queued === false && answer.delivered === true
      && answer.proof === "channel"
    : hasExactKeys(answer, successKeys)
      && answer.queued === true && typeof answer.proof === "string"
      && Boolean(answer.proof));
  if (!uncertain && !success) {
    throw new Error("invalid delivery response from the bridge; no delivery was claimed");
  }
  const outcome = uncertain ? JSON.stringify(answer, null, 1) : answer.text;
...
const socketServer = createServer({ allowHalfOpen: true }, (socket) => {
  ...
  socket.on("end", async () => {
    ...
    try {
      ...
      // The kind this listener understood, echoed: a sender that sees no
      // echo knows an older listener will show its words as Codex's.
      socket.end(JSON.stringify({
        ok: true, message_id: messageId,
        sender_kind: payload.sender_kind === "claude" ? "claude" : "codex",
      }));
    } catch (error) {
...
// stdio transport starts consuming, before the claim, before the bind. Registering
...
// Events are not replayed. Another consumer may already have ended stdin while
// automatic identity was being probed, before this module could wire shutdown.
if (process.stdin.readableEnded || process.stdin.destroyed) void shutdown();

// All shutdown state (including socketServer) is initialized before connect
// starts stdin flowing. In particular, EOF during mkdir must set shuttingDown
// now, not disappear before the listeners are installed later in startup.
if (!shuttingDown) await mcp.connect(new StdioServerTransport());
if (!shuttingDown) await mkdir(dirname(socketPath), { recursive: true });

const namedSocketAlreadyLive = Boolean(
  !shuttingDown && nameIsUsable && peerName && await socketIsLive(socketPath));

if (shuttingDown) {
  // Settle identity below so shutdown can finish without ever claiming/binding.
} else if (!nameIsUsable) {
  console.error(
