...
const mcp = new Server(
  { name: "antiphon", version: "0.1.0" },
  {
    ...
    instructions:
      ...
      "send the result back with reply_to_codex. When several Codex peers are " +
      "live the event's sender_alias names which one spoke — pass it back as " +
      "`to`, because an unaddressed reply is refused rather than guessed. A " +
      "null sender_alias means that peer has no name and cannot be replied to " +
      "by name.",
  },
);
  ...
  tools: [
    {
      ...
      description:
        ...
        "to when more than one is live; with a single peer you can leave it out.",
      inputSchema: {
        type: "object",
        properties: {
          text: {
          ...
          to: {
            type: "string",
            description:
              "Alias of the peer to send to. Optional while the target is " +
...
// `peerId` exists so an unnamed session still has a registry key. It is never
// a sender alias: the other side cannot address a reply to a name this process
// invented for itself, so only `peerName` — explicit and validated — is ever
// sent as one.
const peerId = peerName || `claude-${randomUUID().replace(/-/g, "").slice(0, 3)}`;
const senderAlias = nameIsUsable && peerName ? peerName : null;
