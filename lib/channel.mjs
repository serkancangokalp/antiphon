...
import { automaticProofVerdict as sharedProofVerdict, redactPrivate,
         automaticNameFromDigest } from "./identity.mjs";
...
const mcp = new Server(
  { name: "antiphon", version: packageVersion() },
  {
    ...
    instructions:
      ...
      "public alias and the remedy beside it. After a session rotates to a " +
      "new host session, its old automatic alias stops resolving at once and " +
      "its new one is unreachable until a fresh endpoint exists \u2014 in " +
      "practice an MCP reconnect. `status` and `doctor` name the current " +
      "alias beside that remedy; an identity whose owner cannot be proved " +
      "live is counted, never addressed. A valid " +
...
mcp.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    const automaticJoined = !automaticIdentityDigest
      || (automaticIdentityJoined() && automaticProofVerdict() === "READY");
    ...
    throw new Error("Failed to deliver reply to Codex: " + redactPrivate(
      String(error?.stderr || error?.message || error).trim(), 500));
  }
...
async function claimPeer(mode) {
  const claimed = await registryCall("register_peer", false, mode);
  if (claimed) {
    try {
      const record = JSON.parse(readFileSync(join(
        projectDir, ".antiphon", "peers", `claude-${peerId}`, "endpoint.json"),
        "utf8"));
      claimedBirth = typeof record?.birth === "string" ? record.birth : null;
    } catch {
      claimedBirth = null;
    }
  }
