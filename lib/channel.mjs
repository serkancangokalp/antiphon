...
const mcp = new Server(
  { name: "antiphon", version: packageVersion() },
  {
    ...
    instructions:
      ...
      "unnamed sessions leave no routable peer record and cannot be ruled out. A " +
      ...
      "passing `<unnamed>` as `to` is the same as leaving it out. An unnamed " +
      "Codex hook may leave an unnamed Codex observation for local " +
      ...
      "diagnostic identity, not a recipient alias, and is not addressable. Two " +
      "or more positively live candidates make a bare reply refused; restart " +
      "each intended terminal with a distinct ANTIPHON_NAME and address it by " +
      "that name. A valid " +
  ...
  tools: [
    {
      ...
      description:
        ...
        "leaves no routable peer record, so a bare reply is refused as soon as any " +
...
mcp.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    const payload = {
      ...
      sender_reachable: senderReachable,
    };
...
async function runRegistryCall(subcommand, quiet = false) {
...
function registryCall(subcommand, quiet = false) {
  ...
  registryMutations = registryMutations.then(async () => {
    ...
    const result = await runRegistryCall(subcommand, quiet);
...
if (!nameIsUsable) {
  const configuredName = SESSION_ID_PATTERN.test(peerName)
    ? "ANTIPHON_NAME is a UUID-shaped configured value"
    : `ANTIPHON_NAME=${peerName}`;
  console.error(
    `antiphon: ${configuredName} is not a usable peer name ` +
